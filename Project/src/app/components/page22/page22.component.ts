...
// --------------------------------------------
  ...
  constructor(
    ...
    private page22serviceService: Page22serviceService,
  ...
  onSubmit() {
    if (this.formData.valid) {
      ...
      this.addictName = this.sharedService.getAddictName();
      // const dataToSend = { ...this.formData.value, component: 'page22' };

      this.page22serviceService.saveFormData(this.formData.value, this.addictName).subscribe(
        (response) => {
  ...
  fetchDataByAddictName() {
    if (this.addictName) {
      this.page22serviceService.getFormDataByAddictName(this.addictName).subscribe(
        (data) => {
  ...
  navigateToPage23Page() {
    // Only navigate to the page23 page if data is fetched
    if (this.dataFetched) {
      this.router.navigate(['../page23'], { relativeTo: this.route });
    }
  }
