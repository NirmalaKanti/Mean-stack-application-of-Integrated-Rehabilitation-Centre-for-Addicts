...
export class Page15Component {
  ...
  constructor(
    ...
    private page15serviceService: Page15serviceService,
  ...
  onSubmit() {
    if (this.formData.valid) {
      ...
      this.addictName = this.sharedService.getAddictName();
      // const dataToSend = { ...this.formData.value, component: 'page15' };

      this.page15serviceService.saveFormData(this.formData.value, this.addictName).subscribe(
        (response) => {
  ...
  fetchDataByAddictName() {
    if (this.addictName) {
      this.page15serviceService.getFormDataByAddictName(this.addictName).subscribe(
        (data) => {
  ...
  navigateToPage19Page() {
    // Only navigate to the page19 page if data is fetched
    if (this.dataFetched) {
      this.router.navigate(['../page19'], { relativeTo: this.route });
    }
  }
