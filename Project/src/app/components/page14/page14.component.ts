...
export class Page14Component {
  ...
  constructor(
    ...
    private page14serviceService: Page14serviceService,
  ...
  onSubmit() {
    if (this.formData.valid) {
      ...
      this.addictName = this.sharedService.getAddictName();
      // const dataToSend = { ...this.formData.value, component: 'page14' };

      this.page14serviceService.saveFormData(this.formData.value, this.addictName).subscribe(
        (response) => {
  ...
  fetchDataByAddictName() {
    if (this.addictName) {
      this.page14serviceService.getFormDataByAddictName(this.addictName).subscribe(
        (data) => {
  ...
  navigateToPage15Page() {
    // Only navigate to the page15 page if data is fetched
    if (this.dataFetched) {
      this.router.navigate(['../page15'], { relativeTo: this.route });
    }
  }
